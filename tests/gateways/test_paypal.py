...
@pytest.fixture
def headers():
    return {"User-Agent": settings.PAYPAL_REQUEST_USER_AGENT}


def test_empty_post_to_paypal_gateway(client, headers):
    response = client.post("/gateway/paypal", headers=headers)
    assert response.status_code == 400


def test_post_with_incorrect_paypal_user_agent_accepted(client, ipn_message):
    response = client.post(
        "/gateway/paypal",
        headers={"User-Agent": "Not Paypal User-Agent"},
        data=ipn_message,
    )
    assert response.status_code == 400


def test_post_with_correct_paypal_user_agent_accepted(
    mocker, client, headers, ipn_message
):
    paypal_handshake = mocker.patch("finance_api.gateways.paypal.paypal_handshake")

    response = client.post("/gateway/paypal", headers=headers, data=ipn_message,)
    assert response.status_code == 200


def test_valid_paypal_request_trigger_handshake_process(
    mocker, client, headers, ipn_message
):
    paypal_handshake = mocker.patch("finance_api.gateways.paypal.paypal_handshake")
...
def test_raise_error_if_request_to_not_intended_account(
    mocker, client, headers, ipn_message
):
...
def test_paypal_handshake_send_request_to_verify_url(
...
def test_paypal_after_verified_store_notification_content(mocker, client, ipn_message):
    ...
    mock_store_notification = mocker.patch("finance_api.gateways.paypal.store_notification")
...
def test_log_error_when_paypal_replied_with_invalid_message_received(mocker, client, ipn_message):
    mock_response = mocker.patch("finance_api.gateways.paypal.requests.post")
